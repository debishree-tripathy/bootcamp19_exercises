...
TASK_DEFINITION=$(aws ecs describe-task-definition --task-definition apis)
...
aws ecs register-task-definition --family apis --requires-compatibilities FARGATE --network-mode awsvpc --task-role-arn $TASK_EXEC_ROLE_ARN --execution-role-arn $TASK_EXEC_ROLE_ARN --cpu ${TASK_CPU} --memory ${TASK_MEMORY} --container-definitions "${CONTAINER_DEFINITIONS}"

# Update service to use new task defn - This should pick the new image for the new revision of task defn
aws ecs update-service --cluster "${CLUSTER}" --service "${SERVICE}"  --task-definition apis
